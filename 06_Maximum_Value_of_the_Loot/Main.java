...
public class Main {
    ...
    public static double maxValue(List<Containers> a, int c) {
        ...
        for (int x = 0; x < a.size(); x++) {
            if (Capacity <= 0) {
                break;
            } // If Capacity is already 0, no need to go through the other items.
          // Need to check Capacity here in case we reach 0 Capacity while weights[x] is still greater than 0.
            while (a.get(x).getWeight() > 0 && Capacity > 0) { // Need to check Capacity here in case we reach 0 Capacity while weights[x] is still greater than 0.
